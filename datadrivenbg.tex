...
\begin{figure}[ht!]
  ...
  \caption{Number of loose versus number of tight muons as measured in data. T - Tight, L - Loose, SF - Single-fake, DF - Double-fake, ANA - Analysis sample (Signal region), X - Not used.}
  \label{fig:nloosetight}
\end{figure}

To measure the fake rate independently from the final distributions, an orthogonal sample needs to be constructed. Much like the control regions being defined by an inverted MET requirement, the number of loose muons $N_{\text{loose }\mu}$ in an event can be used. Disjointed from the analysis sample, the fraction of tight to loose muons $F_R = \frac{T}{T+L}$\footnote{This is often confusingly called the ``fake rate'', thus naming the method.} can be determined (C.f. fig.~\ref{fig:nloosetight}). To account for possible dependencies introduced by the detector, it is measured in both $\eta$ and $p_{\text{T}}$. 
...
\begin{figure}[hb!]
  ...
  \caption{Smuon mass and gaugino mass of the CRBVC, including the data-driven background estimate (Cf.~fig~\ref{fig:sccr_nofakes}).}
  \label{fig:CRBVC}
\end{figure}
